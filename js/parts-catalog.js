/** Part metadata extracted from Unity JetEngine.prefab / ARPartDefinition */
export const PARTS = [
  {
    uid: "Outer_Nozzle",
  ...
  {
    uid: "Turbofan",
    meshNames: ["Turbofan", "Hi_pressure_blades", "Hi_pressure_blades2", "LP_blades"],
    title: "Turbofan",
  ...
  {
    uid: "High_Pressure",
    meshNames: ["High_Pressure_compresor", "Hi_pressure_blades2"],
...
export function findPartDefByObjectName(name) {
  if (!name) return null;
  const n = name.trim();
  return (
    PARTS.find((p) => p.meshNames.some((m) => m === n || n.includes(m))) || null
  );
}
